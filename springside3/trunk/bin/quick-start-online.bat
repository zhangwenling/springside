...
call %MAVEN_BAT% dependency:copy-dependencies -DoutputDirectory=lib -Dsilent=true -Pmodules
call %MAVEN_BAT% dependency:copy-dependencies -DoutputDirectory=lib -DexcludeScope=runtime -Dsilent=true -Pexamples
call %MAVEN_BAT% dependency:copy-dependencies -DoutputDirectory=webapp/WEB-INF/lib -DincludeScope=runtime -Dsilent=true -Pexamples

echo [Step 5] 安装SpringSide3 modules 和archetypes到 本地Maven仓库.
call %MAVEN_BAT% clean install  -Pmodules -Dmaven.test.skip=true
 
echo [Step 6] 为Mini-Service 初始化数据库、编译、打包.
cd examples\mini-service
call %MAVEN_BAT% clean package -Pinitdb -Dmaven.test.skip=true
