...
echo [Step 3] 请将servers/tomcat/ci-config/conf/tomcat-user.xml 复制到你的tomcat目录，然后手工启动tomcat(端口为8080)，完成后请按任意键继续
pause
...
call mvn clean compile war:exploded tomcat:exploded -Pinitdb  -Pnotest
cd ..\..\
...
call mvn dependency:copy-dependencies -DoutputDirectory=webapp/WEB-INF/lib  -DincludeScope=runtime
call mvn tomcat:undeploy
call mvn clean compile war:exploded tomcat:exploded  -Pnotest
cd ..\..\
